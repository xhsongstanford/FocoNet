...
def PTB2Kagan(label, predictions):
	...
	the[:, 0] = torch.arccos(torch.clamp(( pp + tt + bb - 1)/2., -0.9999, 0.9999))
	the[:, 1] = torch.arccos(torch.clamp(( pp - tt - bb - 1)/2., -0.9999, 0.9999))
	the[:, 2] = torch.arccos(torch.clamp((-pp - tt + bb - 1)/2., -0.9999, 0.9999))
	the[:, 3] = torch.arccos(torch.clamp((-pp + tt - bb - 1)/2., -0.9999, 0.9999))
