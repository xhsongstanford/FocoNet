...
class Solver(object):
    ...
    def _initialize(self):
        ...
        #self.optimizer = torch.optim.SGD(self.Focnet.parameters(), lr=config.LR, weight_decay=1e-6, momentum=0.9, nesterov=True)
        self.optimizer = torch.optim.Adam(self.Focnet.parameters(), lr=config.LR, betas = (0.9, 0.999), weight_decay=1e-6)
        ...
        if self.multigpu:
            self.Focnet = nn.DataParallel(self.Focnet, device_ids=self.args.gpus)

        self.Focnet.to(device)
    ...
    def set_mode(self, mode):
        ...
        if mode == 'train':
            ...
            self.testloader = DataLoader(self.testset, batch_size=self.testset.N, shuffle=False, drop_last=True, **kwargs)
            self.devloader = DataLoader(self.devset, batch_size=min(self.devset.N, 1024), shuffle=False, drop_last=False, **kwargs)
    ...
    def train(self, finetune=False) :
        ...
        for epoch in range(self.start_epoch, config.NUM_EPOCHS):
            ...
            for i, data in enumerate(self.dataloader):
                ...
                outputs = self.Focnet(waveform, sta_param, sta_num, sta_mask)

                self.optimizer.zero_grad()

                loss = Kagan.PTB2Kagan(focals, outputs).sum().to(device)/len(focals[:, 0, 0])

                # print(torch.sqrt(sta_num))
                # print(Kagan.PTB2Kagan(focals, outputs).to(device) * torch.sqrt(sta_num).T)
                ...
                for param in self.Focnet.parameters():
                    weights += 0.005 * param.norm(2) ** 2
                #print(torch.nn.utils.weight_norm(self.Focnet).weight_g)
                
                loss.backward()
                clip_grad_norm_(self.Focnet.parameters(), 10)
            ...
            if self.curr_epoch % 50 == 0 or dev_loss.item()*180/3.1415 < 19:
                ...
                torch.save(self.Focnet.state_dict(), self.model_savepath +'/'+ self.Focnet.__class__.__name__ + '_' + str(self.curr_epoch) + '.pth')
    ...
    def validate(self):
        ...
        for i, data in enumerate(self.testloader):
            ...
            sta_num = data['sta_num'].to(device)
            sta_mask = data['sta_mask'].to(device)
            outputs = self.Focnet(polarities, sta_param, sta_num, sta_mask).to(device)
            ...
            test_loss += Kagan.PTB2Kagan(focals, outputs).sum().to(device)/len(focals[:, 0, 0])
        ...
        for i, data in enumerate(self.devloader):
            ...
            dev_loss += Kagan.PTB2Kagan(focals, outputs).sum().to(device)/len(focals[:, 0, 0])
        dev_loss = dev_loss / len(self.devloader)
    ...
    def evalidate(self):
        ...
        for i, data in enumerate(self.testloader):
            ...
            with torch.no_grad():
                ...
                outputs = self.Focnet(polarities, sta_param, sta_num, sta_mask, mode='test')#.to(device)
